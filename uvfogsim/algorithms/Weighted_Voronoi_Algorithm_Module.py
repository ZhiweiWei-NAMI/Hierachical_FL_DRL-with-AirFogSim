...
class Weighted_Voronoi_Algorithm_Module(Base_Algorithm_Module):
    def __init__(self, env, training = True, model_path = None):
        ...
        self.max_weight_bs = 100
        self.n_agent = env.n_UAV + env.n_BS
        #  + env.n_BS
        input_states = 231 # (50 + 6 + 4) * 3 + 50 + 1
        ...
        self.batch_size = 512
        self.transition_dicts = [{'actions_weight':deque([],maxlen=self.buffer_size),'states': deque([],maxlen=self.buffer_size),'actions1': deque([],maxlen=self.buffer_size),'actions2': deque([],maxlen=self.buffer_size),'actions3': deque([],maxlen=self.buffer_size),'next_states': deque([],maxlen=self.buffer_size),'rewards': deque([],maxlen=self.buffer_size),'dones': deque([],maxlen=self.buffer_size),'log_probs': deque([],maxlen=self.buffer_size)} for i in range(self.n_agent)]
        self.n_actions = np.array([4, 3, 5]) # 平面方向，每15度一个动作，共24个动作，速度一共有10档，共10个动作，权重调节一共有10档，共10个动作，共28个动作
        self.agents_weight = VDN_Agent(device, self.n_agent, input_states, self.n_actions[-1]+1, epsilon=0.1, gamma=0, learning_rate=1e-3)
    ...
    def act_weights(self, states = None):
        actions = self.agents_weight.select_actions(states)
        uav_weights = [actions[i] / self.n_actions[-1] * self.max_weight for i in range(self.n_UAV)]
        bs_weights = [actions[i+self.n_UAV] / self.n_actions[-1] * self.max_weight_bs for i in range(self.n_BS)]
    ...
    def act_mobility(self, env, vehicle_ids, vehicle_positions, states = None):
        ...
        for i in range(env.n_UAV):
            ...
            action1, action2, action3, log_prob = self.agents_uav[0].take_action(state_i, other_states, selfidx) 
        ...
        for i in range(env.n_BS):
            ...
            action1, action2, action3, log_prob = self.agents_bs[0].take_action(state_i, other_states, selfidx) 
            actions1.append(action1)
            actions2.append(action2)
            actions3.append(action3)
            # action3 = action3 / self.n_actions[2] * self.max_weight_bs
            action3 = np.zeros_like(action3) * self.max_weight_bs
    ...
    def update_agent_weight(self):
        ...
        #     return
        for i in range(5):
            batch_idx = np.random.choice(np.arange(len(self.transition_dicts[0]['states'])), size=self.batch_size, replace=True)
            loss_ = self.agents_weight.update(self.transition_dicts, batch_idx)
            self.actor_loss.append(np.mean(loss_))
    def update_agent(self):
        ...
        for i in range(self.n_agent):
            ...
            if i < self.n_UAV:
                for j in range(3):
                    batch_idx = np.arange(200) + np.random.randint(0,10)*200
                    loss_a, loss_c = self.agents_uav[0].update(self.transition_dicts[i], batch_idx, self.transition_dicts, i)
